...
export default class HeaderListener extends EventEmitter {
    ...
    rangeSize: number = parseInt(process.env.RANGE_SIZE as string);
    ...
    // might need to add a mutex lock here, because if the we run into thw situation that 2 submissions run in parallel this messes stuff up
