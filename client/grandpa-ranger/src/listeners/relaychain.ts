...
import { fetchMissingAuthorityUpdateJustifications } from './justificationCatchup'
...
export default class RelaychainListener extends EventEmitter {
    ...
    async setup(url: string, gatewayId: string, gatewayHeight: number, subscanEndpoint: string) {
        this.gatewayId = gatewayId
        this.api = await ApiPromise.create({
            provider: new WsProvider(url),
        })
        await this.fetchMissingHeaders(gatewayHeight, subscanEndpoint)


        // fetchAuthorityUpdateEvents(subscanEndpoint)
