...
import { fetchGatewayHeight } from "./util";
    ...
    async initializeRelaychainListeners() {
        for (let i = 0; i < config.relaychains.length; i++) {
            ...
            instance.on("RestoreMissingHeaders", (entries: any) => {
                ...
                this.relayer.restoreMissingHeaders(
                    entries.gatewayId,
                    entries.data
                )
                .catch((err: any) => {
                    console.log(err)
                    console.log("Header restoring failed! Shutting down!")
                    process.exit(0)
                })
            })
            ...
            await instance.setup(entry.rpc, entry.id, height, entry.subscanEndpoint)
