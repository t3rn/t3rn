...
async function grandpaDecode(justification) {
 ...
 function createGatewayGenesisConfig(
  metadata,
  genesisHash,
  circuitApi
){
 ...
 function createGatewaySysProps(
  api,
  ss58Format,
  tokenSymbol,
  tokenDecimals
...
async function register(circuit, target)  {
    ...
    return new Promise(async (res, rej) => {
      let listener = await api.rpc.grandpa.subscribeJustifications(async (justification) => {
...
async function registerRococoGateway() {
