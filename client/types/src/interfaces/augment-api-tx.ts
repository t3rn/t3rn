...
import type { Bytes, Compact, Option, Struct, U8aFixed, Vec, bool, u128, u32, u64 } from '@polkadot/types-codec';
  ...
  export interface AugmentedSubmittables<ApiType extends ApiTypes> {
    balances: {
      ...
      forceTransfer: AugmentedSubmittable<
        (source: MultiAddress, dest: MultiAddress, value: Compact<u128>) => SubmittableExtrinsic<ApiType>,
       ...
       *
       * This will alter `FreeBalance` and `ReservedBalance` in storage. it will also alter the total issuance of the
       * system (`TotalIssuance`) appropriately. If the new free or reserved balance is below the existential deposit,
       * it will reset the account nonce (`frame_system::AccountNonce`).
       *
       * The dispatch origin for this call is `root`.
       */
      setBalance: AugmentedSubmittable<
        (who: MultiAddress, newFree: Compact<u128>, newReserved: Compact<u128>) => SubmittableExtrinsic<ApiType>,
        [MultiAddress, Compact<u128>, Compact<u128>]
      >;
      /**
       * Transfer some liquid free balance to another account.
       *
       * `transfer` will set the `FreeBalance` of the sender and receiver. If the sender's account is below the
       * existential deposit as a result of the transfer, the account will be reaped.
       *
       * The dispatch origin for this call must be `Signed` by the transactor.
       *
       * # <weight>
       *
       * - Dependent on arguments but not critical, given proper implementations for input config types. See related
       *   functions below.
       ...
       * - `transfer_keep_alive` works the same way as `transfer`, but has an additional check that the transfer will not
      ...
      transfer: AugmentedSubmittable<
        (dest: MultiAddress, value: Compact<u128>) => SubmittableExtrinsic<ApiType>,
       ...
       *   deposit, which will guarantee to keep the sender account alive (true). # <weight>
    ...
    circuit: {
      bondInsuranceDeposit: AugmentedSubmittable<
        (xtxId: H256 | string | Uint8Array, sideEffectId: H256 | string | Uint8Array) => SubmittableExtrinsic<ApiType>,
        [H256, H256]
      >;
      ...
      confirmSideEffect: AugmentedSubmittable<
        ...
        [H256, T3rnPrimitivesSideEffect, T3rnPrimitivesSideEffectConfirmedSideEffect, Option<Vec<Bytes>>, Option<Bytes>]
      >;
      onExtrinsicTrigger: AugmentedSubmittable<
        (sideEffects: Vec<T3rnPrimitivesSideEffect>, fee: u128, sequential: bool) => SubmittableExtrinsic<ApiType>,
        [Vec<T3rnPrimitivesSideEffect>, u128, bool]
      >;
      /** Used by other pallets that want to create the exec order */
      onLocalTrigger: AugmentedSubmittable<() => SubmittableExtrinsic<ApiType>, []>;
    ...
    grandpa: {
      /**
       * Note that the current authority set of the GRANDPA finality gadget has stalled. This will trigger a forced
       * authority set change at the beginning of the next session, to be enacted `delay` blocks after that. The delay
       * should be high enough to safely assume that the block signalling the forced change will not be re-orged (e.g.
       * 1000 blocks). The GRANDPA voters will start the new authority set using the given finalized block as base. Only
       * callable by root.
       */
      noteStalled: AugmentedSubmittable<
        (delay: u32, bestFinalizedBlockNumber: u32) => SubmittableExtrinsic<ApiType>,
        [u32, u32]
      >;
      /**
    ...
    multiFinalityVerifierEthereumLike: {
       ...
       *
       * The initial configuration provided does not need to be the genesis header of the bridged chain, it can be any
       * arbirary header. You can also provide the next scheduled set change if it is already know.
       *
       * This function is only allowed to be called from a trusted origin and writes to storage with practically no
       * checks in terms of the validity of the data. It is important that you ensure that valid data is being passed in.
       */
      ...
      setOperational: AugmentedSubmittable<
        (operational: bool, gatewayId: U8aFixed | string | Uint8Array) => SubmittableExtrinsic<ApiType>,
        [bool, U8aFixed]
      >;
      /**
       * Change `PalletOwner`.
      ...
      setOwner: AugmentedSubmittable<
        (newOwner: Option<AccountId32>, gatewayId: U8aFixed | string | Uint8Array) => SubmittableExtrinsic<ApiType>,
        [Option<AccountId32>, U8aFixed]
      >;
      /**
       * Verify a target header is finalized according to the given finality proof.
       *
       * It will use the underlying storage pallet to fetch information about the current authorities and best finalized
       * header in order to verify that the header is finalized.
       *
    ...
    system: {
      ...
      killPrefix: AugmentedSubmittable<
        (prefix: Bytes | string | Uint8Array, subkeys: u32) => SubmittableExtrinsic<ApiType>,
        [Bytes, u32]
      >;
       ...
       * - 1 digest item.
       * - 1 event. The weight of this function is dependent on the runtime, but generally this is very expensive. We will
       *   treat this as a full block.
       *
       * # </weight>
       */
       ...
       * - 1 storage write (codec `O(C)`).
       * - 1 digest item.
       * - 1 event. The weight of this function is dependent on the runtime. We will treat this as a full block. # </weight>
       */
    ...
    xdns: {
      ...
      addSideEffect: AugmentedSubmittable<
        ...
        [U8aFixed, Bytes, Vec<T3rnPrimitivesAbiType>, Vec<Bytes>, Vec<Bytes>, Vec<Bytes>, Vec<Bytes>, Vec<Bytes>]
      >;
      /** Removes a xdns_record from the onchain registry. Root only access. */
      purgeXdnsRecord: AugmentedSubmittable<
        (
          requester: AccountId32 | string | Uint8Array,
          xdnsRecordId: H256 | string | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [AccountId32, H256]
      >;
      ...
      updateTtl: AugmentedSubmittable<
        (gatewayId: U8aFixed | string | Uint8Array, lastFinalized: u64) => SubmittableExtrinsic<ApiType>,
        [U8aFixed, u64]
      >;
      /** Generic tx */
      [key: string]: SubmittableExtrinsicFunction<ApiType>;
