...
export async function estimateGasFee<T extends EstimateParams>({
  target,
  action,
  args,
}: Estimate<T>) {
  switch (target) {
    ...
    case 'sepl':
      return estimateEthActionGasFee(
        ...
        args as EstimateEthActionParams,
      )
    default:
...
export async function estimateBidAmount<T extends EstimateParams>(
  input: Estimate<T>,
  profitMargin: (gasFee: number) => number,
) {
  const gasFee = await estimateGasFee(input)
  return {
    value: gasFee + profitMargin(gasFee),
    symbol: NativeAssetMap.getFor(input.target),
  } as Asset
}
...
export async function estimateMaxReward<T extends EstimateParams>({
  ...
  const targetAmountInBaseAsset =
    (await getPriceForSymbol(targetAsset, baseAsset)) * targetAmount
  const gasFeeInTargetNativeAsset = await estimateGasFee({
    target,
    action,
    args,
  })
  const gasFeeInBaseAsset =
    (await getPriceForSymbol(targetNativeAsset, baseAsset)) *
    gasFeeInTargetNativeAsset
  const executorFeeEstimateInBaseAsset =
    (targetAmountInBaseAsset * overSpendPercent) / 100
  const maxRewardInBaseAsset =
    targetAmountInBaseAsset + executorFeeEstimateInBaseAsset + gasFeeInBaseAsset
  return {
    gasFee: [
      {
