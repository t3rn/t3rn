...
const isPrettyPrintEnabled = process.env.PROFILE  === 'local' || process.env.LOG_PRETTY === 'true'
            ...
            await Promise.all(
                events.map(async (record) => {
                    ...
                    if (event.section == 'attesters') {
                        logger.info(`\t${event.section}:${event.method}:: (phase=${record.phase})`)

                        switch (event.method) {
                            case 'NewAttestationMessageHash': {
                                const [targetId, messageHash, executionVendor] = event.data
                                logger.info(`Received the attestation message hash request to sign`,{targetId: targetId.toString(), messageHash: messageHash.toHex(), executionVendor: executionVendor.toString()} )
								...
								if (executionVendor.toString() == 'Substrate') {
									...
									const privateKey = Buffer.from(hexToU8a(this.config.keys.ethereum.privateKey))

									const sigObj = ethUtil.ecsign(Buffer.from(hexToU8a(messageHash.toHex())), privateKey)
									const signature = ethUtil.toRpcSig(sigObj.v, sigObj.r, sigObj.s)

									const tx = this.circuit.client.tx.attesters.submitAttestation(messageHash, signature, targetId)

									logger.info('Executed', { executionVendor: executionVendor.toString(), targetId: targetId.toString(), messageHash: messageHash.toHex(), hash: tx.hash.toHex()})
								}

                                break
                            }
                            case 'NewTargetProposed': {
                                logger.info(`Received the new target proposed event`)
                                break
                            }
                            default: {
                                break
