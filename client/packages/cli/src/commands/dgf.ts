...
import ora from "ora"
import { ErrorListener, ErrorMode, ListenerEvents, processSfx } from "@/utils/dgf/creation.ts"
...
const batchErrorCreation = async () => {
    ...
    listener.on('event', (eventData) => { processEvent(eventData) })
    ...
    for (const errorMode in ErrorMode) {
        ...
        const hash = await submitSfxRaw(extrinsic, true)
 ...
 */
const processEvent = (eventData: { type: ListenerEvents, data: object, error: ErrorMode }) => {
 ...
 */
const validateExtrinsic = (eventData: { type: ListenerEvents, data: object, error: ErrorMode }) => {
