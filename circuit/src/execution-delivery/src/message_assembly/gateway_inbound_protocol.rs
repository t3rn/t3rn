...
pub trait GatewayInboundProtocol {
    ...
    fn set_storage(&self, key: &[u8; 32], value: Option<Vec<u8>>, gateway_type: GatewayType) -> CircuitOutboundMessage;
    ...
    fn call_static(&self, module_name: &str, fn_name: &str, data: Vec<u8>, to: [u8; 32], value: u128, gas: u64, gateway_type: GatewayType) -> CircuitOutboundMessage;
    ...
    /// stamp_proof - Merkle Path in storage trie (that's where the stamp lands) and block reference
    /// finality_proof - Proof that the block is finalized
    fn call_dirty(&self, module_name: &str, fn_name: &str, data: Vec<u8>, to: [u8; 32], value: u128, gas: u64, gateway_type: GatewayType) -> CircuitOutboundMessage;

    /// Call smart contract behind that gateway in a reversible (escrowed) way. Returns (gets it delivered by relayers):
    /// stamp - Event generated on that chain
    /// stamp_proof - Merkle Path in storage trie (that's where the stamp lands) and block reference
    /// finality_proof - Proof that the block is finalized
    fn call_escrow(&self, module_name: &str, fn_name: &str, data: Vec<u8>, to: [u8; 32], value: u128, gas: u64, gateway_type: GatewayType) -> CircuitOutboundMessage;
    ...
    /// stamp - Event generated on that chain
    /// stamp_proof - Merkle Path in storage trie (that's where the stamp lands) and block reference
    fn custom_call_static(&self, module_name: &str, fn_name: &str, data: Vec<u8>, to: [u8; 32], value: u128, gas: u64, gateway_type: GatewayType ) -> CircuitOutboundMessage;
    ...
    /// stamp_proof - Merkle Path in storage trie (that's where the stamp lands) and block reference
    /// finality_proof - Proof that the block is finalized
    fn custom_call_dirty(&self, module_name: &str, fn_name: &str, data: Vec<u8>, to: [u8; 32], value: u128, gas: u64, gateway_type: GatewayType ) -> CircuitOutboundMessage;
    ...
    /// stamp_proof - Merkle Path in storage trie (that's where the stamp lands) and block reference
    /// finality_proof - Proof that the block is finalized
    fn custom_call_escrow(&self, module_name: &str, fn_name: &str, data: Vec<u8>, to: [u8; 32], value: u128, gas: u64, gateway_type: GatewayType ) -> CircuitOutboundMessage;
    ...
    /// stamp_proof - Merkle Path in storage trie (that's where the stamp lands) and block reference
    /// finality_proof - Proof that the block is finalized
    fn transfer_dirty(&self, to: [u8; 32], value: u128, gas: u64, gateway_type: GatewayType) -> CircuitOutboundMessage;

    /// Transfer balance on a chain behind that gateway in a reversible (escrowed) way. Returns (gets it delivered by relayers):
    /// stamp - Event generated on that chain
    /// stamp_proof - Merkle Path in storage trie (that's where the stamp lands) and block reference
    /// finality_proof - Proof that the block is finalized
    fn transfer_escrow(&self, to: [u8; 32], value: u128, gas: u64, gateway_type: GatewayType) -> CircuitOutboundMessage;
    ...
    /// stamp_proof - Merkle Path in storage trie (that's where the stamp lands) and block reference
    /// finality_proof - Proof that the block is finalized
    fn swap_dirty(&self, to: [u8; 32], value: u128, gas: u64, gateway_type: GatewayType) -> CircuitOutboundMessage;
    ...
    /// stamp_proof - Merkle Path in storage trie (that's where the stamp lands) and block reference
    /// finality_proof - Proof that the block is finalized
    fn swap_escrow(&self, from: [u8; 32], x_token: [u8; 32], y_token: [u8; 32], x_value: u128, y_value: u128, gas: u64, gateway_type: GatewayType) -> CircuitOutboundMessage;
}
