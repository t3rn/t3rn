...
impl PalletAssetsOverlay<Runtime, Balance> for Runtime {
    ...
    fn force_create_asset(
        ...
        log::debug!("t3rn-standalone::force_create_asset");
        log::debug!("t3rn-standalone::asset_id: {asset_id:?}");
        log::debug!("t3rn-standalone::asset_admin: {admin:?}");
        Assets::force_create(
            origin,
            asset_id,
...
parameter_types! {
    pub const CelestiaHeadersToStore: u32 = 1000; // we want a multiple of slots_per_epoch + 1
}

impl pallet_celestia_light_client::Config for Runtime {
    type HeadersToStore = CelestiaHeadersToStore;
    type LightClientAsyncAPI = XDNS;
    type RuntimeEvent = RuntimeEvent;
