    ...
    for TakeFirstAssetTrader<AccountId, FeeCharger, Matcher, ConcreteAssets, HandleRefund>
{
    ...
    fn buy_weight(
        ...
        log::trace!(target: "xcm::weight", "TakeFirstAssetTrader::buy_weight weight: {:?}, payment: {:?}, len: {:?}", weight, payment, payment.len());
        payment.clone().fungible.into_keys().for_each(|key| {
            log::trace!(target: "xcm::weight", "TakeFirstAssetTrader::fungible_key {:?}", key);
        });
        payment.clone().fungible.into_values().for_each(|value| {
            log::trace!(target: "xcm::weight", "TakeFirstAssetTrader::fungible_value {:?}", value);
        });
        //		log::trace!(target: "xcm::weight", "TakeFirstAssetTrader::buy_weight asset: {:?}", asset);
        ...
        log::trace!(target: "xcm::weight", "TakeFirstAssetTrader::first asset {:?}({:?}", first.id, first.fun.encode());

        // Get the local asset id in which we can pay for fees
        let (local_asset_id, _) =
        Matcher::matches_fungibles(&first).map_err(|e| {
            match e {
                xcm_executor::traits::Error::AssetNotFound => log::error!(target: "xcm::weight", "failed to match fungible: assetNotfound"),
                xcm_executor::traits::Error::AccountIdConversionFailed => log::error!(target: "xcm::weight", "failed to match fungible: AccountIdConversionFailed"),
                xcm_executor::traits::Error::AmountToBalanceConversionFailed => log::error!(target: "xcm::weight", "failed to match fungible: AmountToBalance"),
                xcm_executor::traits::Error::AssetIdConversionFailed => log::error!(target: "xcm::weight", "failed to match fungible: AsssetIdCOnversion"),
            }
                ...
                FeeCharger::charge_weight_in_fungibles(local_asset_id, weight)
                    ...
                    .map(|asset_balance| {
                        ...
                        if outstanding_balance.saturating_sub(asset_balance) > minimum_balance {
                            (
                                asset_balance,
                                outstanding_balance.saturating_sub(asset_balance),
                            )
                        }
                        ...
                        // dropped
                            ...
                            (
                                outstanding_balance.saturating_sub(minimum_balance),
                                minimum_balance,
                            )
                        }
                    })?;
