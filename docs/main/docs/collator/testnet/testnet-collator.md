...
This guide outlines the essential minimum of steps required to run a collator for t0rn - a release candidate of t3rn on the Rococo testnet. This guide uses latest `v1.48.4-rc.0` release.
...
curl -sSfL \
  ...
  https://raw.githubusercontent.com/t3rn/t3rn/v1.48.4-rc.0/specs/rococo.raw.json

curl -sSfL \
  -o specs/t0rn.raw.json \
  https://raw.githubusercontent.com/t3rn/t3rn/v1.48.4-rc.0/specs/t0rn.raw.json
```
...
```sh
curl -sSfL \
  https://github.com/t3rn/t3rn/releases/download/v1.48.4-rc.0/t0rn-collator-v1.48.4-rc.0-x86_64-unknown-linux-gnu.gz \
...
docker run -p 33333:33333 -p 33334:33334 -p 9944:9944 -p 9933:9933 \
  -v /node ghcr.io/t3rn/t0rn-collator:v1.48.4-rc.0 \
  --collator \
  --name=<collator-name> \
  --base-path=data \
  --keystore-path=keystore \
