...
This guide outlines the essential minimum of steps required to run a collator for t0rn - a release candidate of t3rn on the Rococo testnet. This guide uses the `v1.10.2-rc.0` release, however always use the latest available version.
...
```sh
curl -sSfL \
  https://github.com/t3rn/t3rn/releases/download/v1.10.2-rc.0/t0rn-collator-v1.10.2-rc.0-x86_64-unknown-linux-gnu.gz \
...
curl -sSfL \
  ...
  https://raw.githubusercontent.com/t3rn/t3rn/v1.10.2-rc.0/specs/rococo.raw.json

curl -sSfL \
  -o ~/t0rn/specs/t0rn.raw.json \
  https://raw.githubusercontent.com/t3rn/t3rn/v1.10.2-rc.0/specs/t0rn.raw.json
```

## Option 2: Pull latest Docker image

```sh
docker pull ghcr.io/t3rn/t0rn-collator:v1.10.2-rc.0
```
...
docker run -p 33333:33333 -p 8833:8833 -p 9933:9933 \
  -v /node ghcr.io/t3rn/t0rn-collator:v1.10.2-rc.0 \
  --collator \
  --name genius \
  --base-path /node \
