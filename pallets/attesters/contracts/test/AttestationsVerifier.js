...
function batchEncodePacked(batch) {
    return ethers.utils.solidityPack(
      ...
      [batch.nexCommittee, batch.bannedCommittee, batch.committedSfx, batch.revertedSfx, batch.index]
    );
}
...
//         const bannedCommittee = [nexCommittee[0], nexCommittee[1], nexCommittee[2]];

//         console.log(bannedCommittee);

//         console.log(committedSfx);
//         console.log(revertedSfx);
...
//         let tx = await attestationsVerifier.receiveAttestationBatch(batch.nexCommittee, batch.bannedCommittee, batch.committedSfx, batch.revertedSfx, batch.index, batchMessageHash, signatures);
...
//         // Create an array of addresses
//         const initialCommittee = wallets.map(wallet => wallet.address);
//         const nexCommittee = wallets_next_committee.map(wallet => wallet.address);
