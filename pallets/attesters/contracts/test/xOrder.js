...
describe('XOrder::ID', function () {
    ...
    beforeEach(async function () {
        ...
        contract = await XOrder.deploy(
            escrowGMPContract.address,
            t3rnVaultContract.address
        );
    ...
    it('Should generate ID correctly from nonce 0', async function () {
        ...
        console.log(
            'ethers.utils.defaultAbiCoder.encode(["address", "uint32"], [owner.address, 0])',
            expected_id_0
        );
    ...
    describe('XOrder::Rewards', function () {
        ...
        before(async function () {
            ...
            initialAddr1Balance = await ethers.provider.getBalance(
                addr1.address
            );
        });
        ...
        it.skip('Should subtract maxReward correctly in USDC', async function () {
            ...
            await USDCContract.connect(addr1).approve(
                contract.address,
                maxRewardUSDC
            );
            // send order with ETH as rewardAsset
        ...
        it.skip('Should emit event with correct arguments', async function () {
            // send order with ETH as rewardAsset
            let params = await getParams();
            let sender = await ethers.provider.getSigner(addr1.address)
                ._address;
        ...
        it.skip('Should revert order and refund user in ETH', async function () {
            ...
            expect(status.toString()).to.equal(
                'true,0x03030303,0x05050505,0x70997970C51812dc3A010C7d01b50e0d17dc79C8,0x70997970C51812dc3A010C7d01b50e0d17dc79C8,10000000000000000000,0x0000000000000000000000000000000000000000,2000000000000000000,1000000000000000000,2'
