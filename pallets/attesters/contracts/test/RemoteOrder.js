...
describe("RemoteOrder::Rewards", function() {
    ...
    it("Should emit event with correct arguments", async function() {
        ...
        await expect(contract.connect(addr1).order(params.destination, params.asset, params.targetAccount, params.amount, params.rewardAssetETH, params.insurance, params.maxRewardETH, { value: params.maxRewardETH }))
            ...
            .withArgs(generateId(sender, await nextBlockNumber()), params.destination, params.asset, params.targetAccount, params.amount, params.rewardAssetETH, params.insurance, params.maxRewardETH, await nextBlockNumber());
    });
    ...
    it("Should set order status to Committed after commit", async function() {
        ...
        let id = generateId(addr1.address, await nextBlockNumber());
        await contract.connect(addr1).order(params.destination, params.asset, params.targetAccount, params.amount, params.rewardAssetETH, params.insurance, params.maxRewardETH, { value: params.maxRewardETH });

        let statusPriorCommit = await contract.orders(id);
        expect(statusPriorCommit.toString()).to.equal('true,0x03030303,0x05050505,0x70997970C51812dc3A010C7d01b50e0d17dc79C8,0x70997970C51812dc3A010C7d01b50e0d17dc79C8,10000000000000000000,0x0000000000000000000000000000000000000000,2000000000000000000,1000000000000000000,0'); // 0 corresponds to "Accepted"

        await contract.connect(addr1).commit(id);

        let statusPostCommit = await contract.orders(id);
        expect(statusPostCommit.toString()).to.equal('true,0x03030303,0x05050505,0x70997970C51812dc3A010C7d01b50e0d17dc79C8,0x70997970C51812dc3A010C7d01b50e0d17dc79C8,10000000000000000000,0x0000000000000000000000000000000000000000,2000000000000000000,1000000000000000000,1'); // 1 corresponds to "Committed"
    });
