...
async function main() {
    ...
    console.log(
        'Contract t3rnVault deployed to address:',
        contractt3rnVault.address
    );

    const ContractEscrowGMP = await ethers.getContractFactory('EscrowGMP');
    const contractEscrowGMP = await ContractEscrowGMP.deploy(
        contractt3rnVault.address
    ); // contractt3rnVault.address
    await contractEscrowGMP.deployed();

    console.log(
        'Contract EscrowGMP deployed to address:',
        contractEscrowGMP.address
    );

    const ContractXOrder = await ethers.getContractFactory('XOrder');
    const contractxOrder = await ContractXOrder.deploy(
        contractEscrowGMP.address,
        contractt3rnVault.address
    );
    ...
    const ContractLocalExchange = await ethers.getContractFactory(
        'LocalExchange'
    );
    ...
    console.log(
        'Contract LocalExchange deployed to address:',
        contractLocalExchange.address
    );

    const ContractAttesters = await ethers.getContractFactory(
        'AttestationsVerifierProofs'
    );
    const contractAttesters = await ContractAttesters.deploy(
        [],
        [],
        1,
        contractEscrowGMP.address
    ...
    console.log(
        'Contract AttestationsVerifierProofs deployed to address:',
        contractAttesters.address
    );
}
