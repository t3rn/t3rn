...
impl<T: Config, const TEST_ALL_STEPS: bool> Migration<T, TEST_ALL_STEPS> {
    ...
    pub(crate) fn migrate(weight_limit: Weight) -> (MigrateResult, Weight) {
        ...
        if weight_left
            .checked_reduce(T::WeightInfo::migrate())
            .is_none()
        {
...
mod test {
    ...
    fn is_upgrade_supported_works() {
        ...
        assert!(Migrations::is_upgrade_supported(
        ...
        ExtBuilder::default()
            ...
            .execute_with(|| {
                ...
                for (version, status) in [
                    (1, MigrateResult::InProgress { steps_done: 1 }),
                    (2, MigrateResult::Completed),
                ] {
