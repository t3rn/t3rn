    ...
    BalanceOf<T>: TryFrom<U256> + Into<U256>,
{
        ...
        ) -> (ExitReason, R),
    {
        ...
        // Since evm has other exceptional use cases, we cant be optimistic with remuneration until we know
        // the execution is successful.
        if let ExitReason::Succeed(_) = reason {
            if let Some(vm_info) = threevm_info {
                T::ThreeVm::try_remunerate(
    ...
    BalanceOf<T>: TryFrom<U256> + Into<U256>,
{
    ...
    fn transfer(&mut self, transfer: Transfer) -> Result<(), ExitError> {
        let source = T::AddressMapping::get_or_into_account_id(&transfer.source);
        let target = T::AddressMapping::get_or_into_account_id(&transfer.target);
