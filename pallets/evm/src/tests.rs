...
//
// // use super::*;
// use circuit_mock_runtime::pallet_3vm_evm::Runner;
// pub use circuit_mock_runtime::{
//     pallet_3vm_evm::AddressMapping, Balances, ConfigEvm, Evm as EVM, Runtime as Test,
//     RuntimeOrigin, *,
// };
...
// use frame_support::{
//     assert_ok,
//     traits::{Currency, GenesisBuild, LockIdentifier, LockableCurrency, WithdrawReasons},
// };
...
// //                 let addr = <Test as ConfigEvm>::AddressMapping::get_or_into_account_id(addr);
...
// 		let substrate_addr = <Test as ConfigEvm>::AddressMapping::into_account_id(evm_addr);
//
// 		// Seed account
// 		let _ = <Test as ConfigEvm>::Currency::deposit_creating(&substrate_addr, 100);
// 		assert_eq!(Balances::free_balance(&substrate_addr), 100);
//
// 		// Deduct fees as 10 units
// 		let imbalance = <<Test as ConfigEvm>::OnChargeTransaction as OnChargeEVMTransaction<Test>>::withdraw_fee(&evm_addr, U256::from(10), None).unwrap();
// 		assert_eq!(Balances::free_balance(&substrate_addr), 90);
//
// 		// Refund fees as 5 units
// 		<<Test as ConfigEvm>::OnChargeTransaction as OnChargeEVMTransaction<Test>>::correct_and_deposit_fee(&evm_addr, U256::from(5), imbalance);
...
//
//         let _ =
//             <Test as ConfigEvm>::Currency::deposit_creating(&substrate_addr, 21_777_000_000_000);
...
//         // Drain funds
//         let _ =
//             <<Test as ConfigEvm>::OnChargeTransaction as OnChargeEVMTransaction<Test>>::withdraw_fee(
//                 &evm_addr,
//                 U256::from(100),
//             )
//             .unwrap();
...
//         assert_eq!(
//             <Test as ConfigEvm>::Currency::deposit_into_existing(&substrate_addr, 5u32.into())
//                 .is_err(),
//             true
//         );
...
//         let account_id = <Test as ConfigEvm>::AddressMapping::into_account_id(evm_addr);
...
//     ExtBuilder::default().build().execute_with(|| {
//         let before_tip = <Test as ConfigEvm>::Currency::total_issuance();
//         let result = EVM::call(
//             RuntimeOrigin::root(),
//             H160::default(),
//             H160::from_str("1000000000000000000000000000000000000001").unwrap(),
...
//         );
//         result.expect("EVM can be called");
//         let after_tip = <Test as ConfigEvm>::Currency::total_issuance();
//         // Only base fee is burned
//         let base_fee: u64 = <Test as ConfigEvm>::FeeCalculator::min_gas_price()
//             .0
...
//             Vec::new(),
//         );
//         let (base_fee, _) = <Test as ConfigEvm>::FeeCalculator::min_gas_price();
...
//         let substrate_addr: <Test as frame_system::ConfigEvm>::AccountId =
//             <Test as ConfigEvm>::AddressMapping::into_account_id(addr);
//         let substrate_addr_2: <Test as frame_system::ConfigEvm>::AccountId =
//             <Test as ConfigEvm>::AddressMapping::into_account_id(addr_2);
//
...
//             U256::zero()
//         );
//         let _ = <Test as ConfigEvm>::Runner::call(
//             non_balance_account,
//             H160::from_str("1000000000000000000000000000000000000001").unwrap(),
//             Vec::new(),
//             U256::from(1u32),
...
//             &<Test as ConfigEvm>::config().clone(),
//         )
...
//             U256::zero()
//         );
//         let res = <Test as ConfigEvm>::Runner::call(
//             non_balance_account,
//             H160::from_str("1000000000000000000000000000000000000001").unwrap(),
//             Vec::new(),
//             U256::from(1u32),
...
//         match <Test as ConfigEvm>::Runner::call(
//             // Contract address.
//             H160::from_str("1000000000000000000000000000000000000001").unwrap(),
//             H160::from_str("1000000000000000000000000000000000000001").unwrap(),
//             Vec::new(),
...
//
//         // internal call
//         assert!(<Test as ConfigEvm>::Runner::call(
//             // Contract address.
//             H160::from_str("1000000000000000000000000000000000000001").unwrap(),
//             H160::from_str("1000000000000000000000000000000000000001").unwrap(),
//             Vec::new(),
