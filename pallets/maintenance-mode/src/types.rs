    ...
    T: Config,
{
    ...
    fn pre_upgrade() -> Result<Vec<u8>, sp_runtime::DispatchError> {
        if Pallet::<T>::maintenance_mode() {
            Ok(T::MaintenanceExecutiveHooks::pre_upgrade()?)
        } else {
            Ok(T::NormalExecutiveHooks::pre_upgrade()?)
        }
    }
    ...
    fn post_upgrade(state: Vec<u8>) -> Result<(), sp_runtime::DispatchError> {
        if Pallet::<T>::maintenance_mode() {
            Ok(T::MaintenanceExecutiveHooks::post_upgrade(state)?)
        } else {
            Ok(T::NormalExecutiveHooks::post_upgrade(state)?)
        }
    }
}
