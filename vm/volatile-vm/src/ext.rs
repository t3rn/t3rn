...
pub struct DefaultRuntimeEnv2<T: EscrowTrait + SystemTrait + VolatileVM> {
...
pub struct DefaultRuntimeEnv<'a, T: EscrowTrait + SystemTrait + VolatileVM> {
    ...
    T: EscrowTrait + SystemTrait + VolatileVM,
{
    ...
    fn deposit_event(&mut self, topics: Vec<TopicOf<Self::T>>, data: Vec<u8>) {
        <system::Pallet<T>>::deposit_event_indexed(
            &*topics,
            <Self::T as VolatileVM>::Event::from(crate::pallet::Event::VolatileVMEmitted(
                self.escrow_account.clone(),
                self.requester.clone(),
                data,
            ))
...
pub trait ExtStandards {
    type T: EscrowTrait + SystemTrait + VolatileVM;
    /// <h2>Storage</h2>
    ///
    ...
    fn random(&self, subject: &[u8]) -> (SeedOf<Self::T>, BlockNumberOf<Self::T>) {
        <Self::T as VolatileVM>::Randomness::random(subject)
    }
