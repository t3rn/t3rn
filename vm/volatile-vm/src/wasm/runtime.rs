...
use crate::{ExecResult, StorageKey, TopicOf};
    ...
    E: Ext + 'a,
{
        ...
        R: AsRef<[u8]>,
    {
        ...
        // let hash_ref: sp_std::vec::Vec<u8> = self.read_sandbox_memory_as(hash)?;
        // let hash_ref: sp_std::vec::Vec<u8> = Decode::decode(&mut &hash[..]).map_err(Into::into)?;
    ...
    fn instantiate(
        ...
        let instantiate_outcome = self
            .ext
            .instantiate(gas, code_hash, value, input_data, &salt);
    ...
    fn terminate(&mut self, beneficiary_ptr: u32) -> Result<(), TrapReason> {
        ...
        let beneficiary: <<E as Ext>::T as system::Config>::AccountId =
    ...
    fn restore_to(
        ...
        let dest: <<E as Ext>::T as system::Config>::AccountId =
