...
use crate::{
    ...
    wasm::prepare::ChainId,
    CodeHash, Config, ExecResult, Schedule,
};
use sp_std::prelude::*;
...
/// Run mode for contract
#[derive(Clone, Encode, Decode, PartialEq)]
pub enum RunMode {
    /// Dry-run to establish whether the code is correct and its execution steps
    /// Output: Vec<OutboundMessages>
    /// Effects: Stores code as a result if unseen and correct
    Dry,
    /// Pre-run contract to get the expected data on the output
    /// Output: Vec<(OutboundMessages, ExpectedOutput)>
    /// Effects: Commit expected post-run state into separate CommitStorageState set
    Pre,
    /// Post-run contract taking ExpectedOutput as input and validating it against
    /// Can be skipped if able to parse ProvidedOutput and compare with ExpectedOutput directly
    /// Effect: After xtx is finished moves CommitStorageState into StorageState
    Post,
}
...
impl<T: Config> PrefabWasmModule<T> {
    ...
    pub fn from_code(
        ...
        schedule: &Schedule<T>,
        run_mode: crate::wasm::RunMode,
        target_id: Option<ChainId>,
    ) -> Result<Self, DispatchError> {
        prepare::prepare_contract(original_code, schedule, run_mode, target_id).map_err(Into::into)
    }
...
mod tests {
    ...
    use crate::{ExecReturnValue, ReturnFlags};
...
"#;
        ...
        assert_eq!(
            std::str::from_utf8(&ext.debug_buffer).unwrap(),
            "Hello World!"
        );
    }
