...
use crate::{
    ...
    weights::WeightInfo,
    CodeHash, Config, DryRunCodeCandidates, Error, Event, Pallet as VolatileVM, PristineCode,
    Schedule, Weight,
};
...
pub fn store<T: Config>(mut prefab_module: PrefabWasmModule<T>) {
    ...
    let was_newly_inserted_res = VolatileVM::<T>::update_contract_metadata_lazy(
        code_hash,
        Box::new(
            |existing: &mut Option<PrefabWasmModule<T>>| match existing {
                Some(module) => {
    ...
    if let Ok(_) = was_newly_inserted_res {
        VolatileVM::<T>::deposit_event(Event::CodeStored(code_hash.clone()));
    }
}
...
pub fn increment_refcount<T: Config>(
    ...
    gas_meter.charge(CodeToken::UpdateRefcount(estimate_code_size::<T>(
        &code_hash,
    )?))?;

    VolatileVM::<T>::update_contract_metadata_lazy(
        code_hash,
        Box::new(|existing: &mut Option<PrefabWasmModule<T>>| {
...
pub fn decrement_refcount<T: Config>(
    ...
    let update_res = VolatileVM::<T>::update_contract_metadata_lazy(
        code_hash,
        Box::new(|existing: &mut Option<PrefabWasmModule<T>>| {
            if let Some(module) = existing {
...
mod private {
    ...
    pub fn reinstrument<T: Config>(
        _prefab_module: &mut PrefabWasmModule<T>,
...
fn finish_removal<T: Config>(code_hash: CodeHash<T>) {
    ...
    VolatileVM::<T>::deposit_event(Event::CodeRemoved(code_hash))
}
...
fn estimate_code_size<T: Config>(code_hash: &CodeHash<T>) -> Result<u32, DispatchError> {
    let key = <DryRunCodeCandidates<T>>::hashed_key_for(code_hash);
    let mut data = [0u8; 0];
