    ...
    > XExecSignal<AccountId, BlockNumber>
{
    ...
    pub fn generate_id<T: Config, Hasher: sp_core::Hasher>(&self) -> XExecSignalId<T> {
        ...
        let mut system_hash = T::Hash::default();

        system_hash.as_mut().copy_from_slice(&hash.as_ref()[..32]);

        system_hash
        //
        // let mut new_out = XExecSignalId::<T>::default();
        //
        //
        //
        // new_out.clone_from(&mut hash.into());
        //
        // new_out
        // .expect("Failed to convert hash to XExecSignalId")
        // hash.try_into()
        //     .expect("Failed to convert hash to XExecSignalId")
    }
    ...
    pub fn setup_fresh<T: frame_system::Config>(
        ...
        let id = signal.generate_id::<T, Keccak256>();
        (id, signal)
    }
}
...
mod tests {
    ...
    fn sfx_id_is_compatible_with_keccak_and_20b_accounts() {
        ...
        assert_eq!(
            sfx_id,
            hex!("1d139c4395e83a25f2e1ed9b2dfaf406c8c73449eef68799fe8bd7dc7e44ca79").into()
        );
    }
