...
pub mod tests {
    ...
    fn select_new_header_to_download_works_with_empty_queue() {
        ...
        eth_sync
            .headers_mut()
            .header_response(header(11).header().clone());
        eth_sync
            .headers_mut()
            .maybe_extra_response(&header(11).id(), false);
        assert_eq!(eth_sync.select_new_header_to_download(), Some(12));

        // but then queue is drained
        eth_sync
            .headers_mut()
            .target_best_header_response(&header(11).id());
    ...
    fn sync_without_reorgs_works() {
        ...
        eth_sync
            .headers
            .header_response(header(101).header().clone());

        // now header #101 is ready to be submitted
        assert_eq!(
            eth_sync.headers.header(HeaderStatus::MaybeExtra),
            Some(&header(101))
        );
        eth_sync.headers.maybe_extra_response(&id(101), false);
        assert_eq!(
            eth_sync.headers.header(HeaderStatus::Ready),
            Some(&header(101))
        );
        assert_eq!(
            eth_sync.select_headers_to_submit(false),
            Some(vec![&header(101)])
        );
        ...
        eth_sync
            .headers
            .header_response(header(102).header().clone());
    ...
    fn sync_with_orphan_headers_work() {
        ...
        assert_eq!(
            eth_sync.headers.header(HeaderStatus::MaybeOrphan),
            Some(&header(101))
        );
        ...
        assert_eq!(
            eth_sync.headers.header(HeaderStatus::Orphan),
            Some(&header(101))
        );
        eth_sync
            .headers
            .header_response(header(100).header().clone());
    ...
    fn only_submitting_headers_in_backup_mode_when_stalled() {
        ...
        assert_eq!(
            eth_sync.select_headers_to_submit(true),
            Some(vec![&header(101)])
        );
    }
