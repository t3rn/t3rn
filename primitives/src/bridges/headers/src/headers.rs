...
impl<P: HeadersSyncPipeline> QueuedHeaders<P> {
    ...
    pub fn headers_in_status(&self, status: HeaderStatus) -> usize {
        match status {
            ...
            HeaderStatus::MaybeExtra => self
                ...
                .values()
                .fold(0, |total, headers| total + headers.len()),
            HeaderStatus::Extra => self
                .extra
                .values()
                .fold(0, |total, headers| total + headers.len()),
            HeaderStatus::Ready => self
                .ready
                .values()
                .fold(0, |total, headers| total + headers.len()),
            HeaderStatus::Incomplete => self
                .incomplete
                .values()
                .fold(0, |total, headers| total + headers.len()),
            HeaderStatus::Submitted => self
                .submitted
                .values()
                .fold(0, |total, headers| total + headers.len()),
        }
    ...
    pub fn add_incomplete_headers(
        ...
        for new_incomplete_header in new_incomplete_headers {
            ...
            let move_origins = move_origins
                .into_iter()
                .chain(std::iter::once(new_incomplete_header));
    ...
    pub fn incomplete_headers_response(&mut self, ids: HashSet<HeaderIdOf<P>>) {
        ...
        for just_completed_header in just_completed_headers {
            ...
            let move_origins = move_origins
                .into_iter()
                .chain(std::iter::once(just_completed_header));
            for move_origin in move_origins {
                move_header_descendants::<P>(
...
pub(crate) mod tests {
    ...
    fn header_works() {
        ...
        queue
            ...
            .entry(10)
            .or_default()
            .insert(hash(1), header(100));
        assert_eq!(
            queue
                .header(HeaderStatus::MaybeOrphan)
                .unwrap()
                .header()
                .hash,
            hash(100)
    ...
    fn descendants_are_moved_on_substrate_best_header_response() {
        ...
        assert_eq!(
            queue.known_headers[&101][&hash(101)],
            HeaderStatus::MaybeExtra
        );
        assert_eq!(
            queue.known_headers[&102][&hash(102)],
            HeaderStatus::MaybeExtra
        );
        assert_eq!(
            queue.known_headers[&103][&hash(103)],
            HeaderStatus::MaybeExtra
        );
    }
    ...
    fn positive_maybe_orphan_response_works() {
        ...
        assert_eq!(queue.maybe_extra.len(), 3);
        assert_eq!(
            queue.known_headers[&100][&hash(100)],
            HeaderStatus::MaybeExtra
        );
        assert_eq!(
            queue.known_headers[&101][&hash(101)],
    ...
    fn completion_response_works() {
        ...
        queue
            .incomplete_headers
            .insert(id(200), Some(Instant::now()));
        queue
            .incomplete_headers
            .insert(id(300), Some(Instant::now()));
    ...
    fn incomplete_headers_response_works() {
        ...
        assert!(queue
            ...
            .entry(101)
            .or_default()
            .contains_key(&hash(101)));
        assert!(queue
            .incomplete
            .entry(102)
            .or_default()
            .contains_key(&hash(102)));
